...
*/
	...
	quadrotor_ParticleModel = function(){
		...
		this.setModelPosition = function (x, y, z){
			...
			mainMesh.position.set (particleMeshes[i].position.x + positionDiff_X,
								particleMeshes[i].position.y + positionDiff_Y, 
								particleMeshes[i].position.z + positionDiff_Z);
		...
		this.setModelColor = function (colorNum){
			...
			mainMesh.material = modelMaterial;
		}
		...
		this.updateAnimate = function(){
			...
			//update mainMesh
			particles[16].position.copy (mainMesh.position);
			var q = getQuaternion();
			mainMesh.quaternion.set(q.x,q.y,q.z,q.w);
		...
		function constructModelPoint(innerLen, outerLen, height, particalMass){
			...
			q.setFromAxisAngle(new THREE.Vector3( 0, 0, 1 ),Math.PI*0.25);
				
			var boxGeometry = new THREE.CubeGeometry(1.5,1.5, 0.3);
				
			var sideMesh1Geo = new THREE.CubeGeometry(4,0.5,0.3);
			var sideMesh1 = new THREE.Mesh( sideMesh1Geo, material );
			//sideMesh1.position.set(0, 0.15, 0);
			sideMesh1.quaternion.set(q.x,q.y,q.z,q.w);
			sideMesh1.useQuaternion = true;
				
			var sideMesh2Geo = new THREE.CubeGeometry(0.5,4, 0.3);
			var sideMesh2 = new THREE.Mesh( sideMesh2Geo, material );
			//sideMesh2.position.set(0, 0.15, 0);
